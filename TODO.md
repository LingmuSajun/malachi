...
### 3. 鑑定結果を LINE チャットに送り返す【体験の連続性】

- [ ] LIFF 鑑定完了後、`liff.sendMessages()` でカード名+要約を LINE チャットに投稿
- [ ] LINE に残ることで見返せる・友人にシェアしやすくなる
- [ ] Flex Message でカード画像+一言抜粋を美しく表示

### 4. テーマ選択 UI — 質問の質を上げる【LIFF UX 改善】

- [ ] 質問入力前に「恋愛」「仕事・お金」「人間関係」「今日の一枚」のボタンを表示
- [ ] 選択したテーマを `divine()` に渡してマラキの解釈をテーマ特化させる
- [ ] テーマに応じたプレースホルダー文言に切り替え(質問入力のハードルを下げる)
